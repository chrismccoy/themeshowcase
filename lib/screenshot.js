...
/**
 * Restores the timeout error needed.
 */
 ...
 */
async function shootWithPageres({ url, destDir, name, width, height, timeoutMs }) {
  ...
  await new Pageres({
    ...
    .source(url, [`${width}x${height}`], { crop: false })
    .destination(destDir)
    .run();
...
export function createCapture({
  ...
  return async function capture(url, destDir, name) {
    ...
    try {
      await Promise.race([shoot({ url: landing, destDir, name, width, height, timeoutMs }), givesUp]);
    } catch (error) {
