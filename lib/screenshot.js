...
import { FLATTEN_STICKY } from "./flatten-sticky.js";
...
/**
 * Restores the timeout error
 */
export function lendErrorsNamespace(puppeteer) {
  puppeteer.default.errors ??= { TimeoutError: puppeteer.TimeoutError };
}

/**
 * Options for page res
 */
export function sourceOptions(fullPage) {
  return {
    crop: !fullPage,
    script: fullPage ? FLATTEN_STICKY : undefined,
  };
}
 ...
 */
async function shootWithPageres({ url, destDir, name, width, height, timeoutMs, fullPage }) {
  lendErrorsNamespace(await import("puppeteer"));
  ...
  await new Pageres({
    ...
    .source(url, [`${width}x${height}`], sourceOptions(fullPage))
    .destination(destDir)
    .run();
...
export function createCapture({
   ...
   * Captures one page.
   */
  return async function capture(url, destDir, name, { fullPage = true } = {}) {
    ...
    try {
      await Promise.race([
        shoot({ url: landing, destDir, name, width, height, timeoutMs, fullPage }),
        givesUp,
      ]);
    } catch (error) {
